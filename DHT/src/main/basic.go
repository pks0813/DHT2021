...
func basicTest() (bool, int, int) {
	...
	defer func() {
		if r := recover(); r != nil {
			_, _ = fmt.Println("Program panicked with", r)
		}
		panicked = true
	}()
	...
	for t := 1; t <= basicTestRoundNum; t++ {
		_, _ = fmt.Printf("Basic Test Round %d\n", t)
		...
		_, _ = fmt.Printf("Start putting (round %d, part 1)\n", t)
		...
		_, _ = fmt.Printf("Start getting (round %d, part 1)\n", t)
		get1Cnt := 0
		...
		_, _ = fmt.Printf("Start deleting (round %d, part 1)\n", t)
		...
		/* Quit. */
		_, _ = fmt.Printf("Start quitting (round %d)\n", t)
		for i := 1; i <= basicTestRoundQuitNodeSize; i++ {
			...
			time.Sleep(basicTestJoinQuitSleepTime)
			//for j:=0;j< len(nodesInNetwork);j++{
			//	nodes[nodesInNetwork[j]].Print()
			//}
		}
		_, _ = fmt.Printf("Quit (round %d) passed.\n", t)
		time.Sleep(basicTestAfterJoinQuitSleepTime)
		...
		_, _ = fmt.Printf("Start putting (round %d, part 2)\n", t)
		for i := 1; i <= basicTestRoundPutSize; i++ {
			key := randString(lengthOfKeyValue)
			value := randString(lengthOfKeyValue)
			kvMap[key] = value
		...
		_, _ = fmt.Printf("Start getting (round %d, part 2)\n", t)
		get2Cnt := 0
		for key, value := range kvMap {
			ok, res := nodes[nodesInNetwork[rand.Intn(len(nodesInNetwork))]].Get(key)
			if !ok || res != value {
		...
		_, _ = fmt.Printf("Start deleting (round %d, part 2)\n", t)
		for i := 1; i <= basicTestRoundDeleteSize; i++ {
			for key := range kvMap {
				delete(kvMap, key)
				success := nodes[nodesInNetwork[rand.Intn(len(nodesInNetwork))]].Delete(key)
